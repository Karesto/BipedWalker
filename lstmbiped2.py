...
class Policy(nn.Module):
    def __init__(self,h):
        ...
        self.l2 = nn.Linear(self.state_space, h[1])
        self.l3 = nn.Linear(self.state_space, h[2])
        self.l4 = nn.Linear(self.state_space, h[3])
        self.ls = nn.LSTM(h[0] + h[1] + h[2] +h[3],self.action_space)
        self.re = nn.ReLU()
        self.sm = nn.Softmax()
        self.th = nn.Tanh()
        self.gamma = gamma
    ...
    def forward(self, x,h):
        res1 = self.re(self.l1(x))
        res2 = self.re(self.l2(x))
        res3 = self.re(self.l3(x))
        res4 = self.re(self.l4(x))
        res, h = self.ls(torch.cat([res1,res2,res3,res4]).view(1,1,-1),h)
        return self.th(res.view(-1)), h
